...
<body>
	...
	<div class="kotak_login" style="width: 600px; position: absolute; left: 35%; top: 130px; margin: 0; margin-top: 0;">
		<p class="tulisan_login" style="margin-bottom: 25px; font-size: 22px;">Update Periksa</p>

		<form action="update_periksa.php" method="post">
			<?php
			include "koneksi.php";
			$id = $_GET['id'];
			$query = "SELECT * FROM periksa WHERE id_periksa='$id'";
			$sql = mysqli_query($connect, $query);
			while ($data = mysqli_fetch_array($sql)) {
			?>
				<input type="hidden" name="id_periksa" value="<?php echo $data['id_periksa'] ?>">
				
				<label>ID Periksa</label>
				<input type="text" class="form_login" value="<?php echo $data['id_periksa'] ?>" disabled>

				<label>ID Pasien</label>
				<input type="number" name="id_pasien" class="form_login" value="<?php echo $data['id_pasien'] ?>" required>

				<label>Tanggal Periksa</label>
				<input type="date" name="tanggal_periksa" class="form_login" value="<?php echo $data['tanggal_periksa'] ?>" required>

				<label>Keterangan</label>
				<textarea name="keterangan" class="form_login" rows="4" required><?php echo $data['keterangan'] ?></textarea>
